...
\begin{itemize}
  ...
  \item  Reflected -- Typically delivered via email or a neutral web site. It occurs when a
...
\begin{figure}[H]
  ...
  \caption[Common XSS vulnerability flow]{Common XSS vulnerability flow}
  \label{img:xss}
\end{figure}
...
Other attacks may use XSS to change the application content and present falsy information to the
user.
...
The main idea behind Trusted Types is to make the dangerous DOM APIs secure by default and motivate
...
\begin{itemize}
  \item preventing or mitigating server side generated markup attacks -- Defending against XSS on
        ...
        code can run on both client and server, such as NextJS (\ref{intro-nextjs}). To address
        these attacks, use the existing recommended solutions like templating systems or CSP
        \emph{script-src}.
  ...
  \item guarding cross origin JavaScript execution, for example loading new documents via
        \emph{data:} URLs -- Trusted Types do not guard cross origin executions as all
  \item protecting against malicious developers of the web application -- It is implicitly assumed
        that untrusted developer can cause much severe damages. Attempting to guard against
...
default policy defeats enabled all malicious attacker payloads to reach the DOM sinks. Developers
should be very cautious when using the default policy and preferably use it only for a transitional
period until the offending code is refactored not to use the dangerous DOM sinks
\cite{tt_spec:default_policy}.
...
security, but there is no automated way that can assert the web application safety. This means that
...
DOM sinks, either implicitely and explicitely, and whether there is way for attacker to misuse them.
...
proven in various projects or various scale \cite{tt_web_framework_paper}
